...
computeBARTWeighted <- function(trainX, trainY, candidateX, candidateY, num_iterations, save_posterior = FALSE, save_posterior_dir = "bcf/results", num_cv = "default", linear=1, sequential=1) {
  dim <- ncol(trainX)
  ...
  if (save_posterior == TRUE) {
    posterior_samples <- list("posterior_samples" = rowMeans(treatment_effects))
    save(posterior_samples, file = paste(save_posterior_dir, "/posterior_BART_synthetic_", i, "_", num_cv, ".RData", sep=""))
  }
